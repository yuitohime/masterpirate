-- ==========================================
-- DELTA UI V10 - ULTIMATE (ORIGINAL UI + EXACT PATH HAKI/KEN + FLY FIX)
-- ==========================================
...
-- AUTO HAKI / KEN (Đã ghép tọa độ chuẩn tuyệt đối từ Console của bạn)
task.spawn(function()
        ...
        if _G_V10.AutoHaki then
            pcall(function()
                if not char:FindFirstChild("Haki") then
                    -- Tọa độ chuẩn của Haki lấy từ hình ảnh Console
                    local hakiRemote = rs:FindFirstChild("Assets")
                        and rs.Assets:FindFirstChild("Remote")
                        and rs.Assets.Remote:FindFirstChild("RemoteEvent")
                        
                    if hakiRemote then
                        hakiRemote:FireServer("GTS", true, nil, true)
                    end
                end
            end)
        end
        ...
        if _G_V10.AutoKen then
            pcall(function()
                ...
                if not kenNode or (kenNode and kenNode:FindFirstChild("Close")) then
                    -- Tọa độ chuẩn của Ken lấy từ hình ảnh Console
                    local kenRemote = rs:FindFirstChild("Assets")
                        and rs.Assets:FindFirstChild("Remote")
                        and rs.Assets.Remote:FindFirstChild("BindabledEvent")
                        and rs.Assets.Remote.BindabledEvent:FindFirstChild("Ken")
                        
                    if kenRemote then
                        kenRemote:FireServer("GTS", true, nil, true)
                    end
                end
            end)
        end
...
-- LÕI BAY TỰ DO (FIX LỖI ĐỨNG ĐƠ SAU KHI TẮT BAY BẰNG GETTINGUP STATE)
