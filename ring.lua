-- ==========================================
-- DELTA UI V10 - ULTIMATE MAX (ORIGINAL UI + SMART HAKI + SEA + RAID + PRO WEAPON SWAP + BOSS/SPAWN)
-- ==========================================
...
Title.BackgroundTransparency = 1; Title.Text = "AUTO FARM V10 (Ultimate Max)"
...
CreateDropdown(TabBoss, "Chọn Lượng Spawn Mihawk", {"x100", "x10", "x1"}, "MihawkAmount")
...
CreateDropdown(TabBoss, "Chọn Vật Phẩm Give", {"Shadow Spirit", "Rotten Flesh", "Aqua Soul", "Bone", "Blood Bottle"}, "ShadowItem")
CreateDropdown(TabBoss, "Chọn Số Lượng Give", {"x1", "x5", "x10"}, "ShadowAmount")
...
-- ==========================================
-- ENGINE LÕI (VIM ASYNC FIX + CLICK VIRTUAL)
-- ==========================================
...
-- Hàm hỗ trợ bấm nút ảo bằng tên (Button1, Button2, v.v.)
local function ClickDialogButtonByIndex(index)
    local talking = LocalPlayer.PlayerGui:FindFirstChild("Talking")
    if talking then
        local btn = talking:FindFirstChild("Button" .. tostring(index), true)
        if btn then
...
RunService.RenderStepped:Connect(function()
    ...
    if char and char:FindFirstChild("HumanoidRootPart") and char:FindFirstChild("Humanoid") then
        ...
        -- Cập nhật chống kẹt vật lý cho các chức năng Auto
...
task.spawn(function()
    while task.wait() do
        ...
        if isFarmingAction and not _G_V10.FreeFly then
            ...
            -- LOGIC XẢ SKILL RIÊNG BIỆT THEO TỪNG VŨ KHÍ
...
-- ==========================================
-- AUTO RAID (ĐÃ FIX LỖI TỰ ĐỘNG CHỌN NÚT BUTTON 2)
-- ==========================================
local function AutoBuyRaidWithBeli()
    local npcFolder = Workspace:FindFirstChild("NPC")
    local dazzl = npcFolder and npcFolder:FindFirstChild("Dazzl")
    
    if dazzl and dazzl:FindFirstChild("HumanoidRootPart") then
        pcall(function() game:GetService("ReplicatedStorage").Assets.Remote.RemoteFunction.Talking:InvokeServer(dazzl, dazzl, dazzl) end)
    end
    task.wait(0.5) 
    local talkingGui = LocalPlayer:WaitForChild("PlayerGui"):FindFirstChild("Talking")
    if talkingGui then
        local btn2 = talkingGui:FindFirstChild("Button2", true) 
        if btn2 then
            if getconnections then
                for _, c in pairs(getconnections(btn2.MouseButton1Click)) do c:Fire() end
                for _, c in pairs(getconnections(btn2.MouseButton1Down)) do c:Fire() end
            elseif firesignal then firesignal(btn2.MouseButton1Click) end
        end
    end
end
...
task.spawn(function()
    while task.wait(0.5) do
        ...
        if distToReRaid < 3000 then 
            ...
            if _G_V10.AutoBuyRaid then task.spawn(function() AutoBuyRaidWithBeli() end) end
...
-- ==========================================
-- AUTO BOSS & SPAWN LÕI (MIHAWK & SHADOW)
-- ==========================================
task.spawn(function()
    while task.wait(1) do
        local char = LocalPlayer.Character
        ...
        -- SPAWN MIHAWK
        if _G_V10.AutoSpawnMihawk then
            ...
            if npc and npc:FindFirstChild("HumanoidRootPart") then
                ...
                local btnIdx = 3 -- x1 default
                if _G_V10.MihawkAmount == "x100" then btnIdx = 1 elseif _G_V10.MihawkAmount == "x10" then btnIdx = 2 end
                ClickDialogButtonByIndex(btnIdx)
            end
        end

        -- GIVE SHADOW ITEM
        if _G_V10.AutoGiveShadow then
            ...
            if npc and npc:FindFirstChild("HumanoidRootPart") then
                ...
                local itemIdx = 1 -- Shadow Spirit
                if _G_V10.ShadowItem == "Rotten Flesh" then itemIdx = 2
                elseif _G_V10.ShadowItem == "Aqua Soul" then itemIdx = 3
                elseif _G_V10.ShadowItem == "Bone" then itemIdx = 4
                elseif _G_V10.ShadowItem == "Blood Bottle" then itemIdx = 5 end
                
                ClickDialogButtonByIndex(itemIdx)
                task.wait(0.5) -- Đợi bảng số lượng hiện lên
                
                local amtIdx = 1 -- x1
                if _G_V10.ShadowAmount == "x5" then amtIdx = 2 elseif _G_V10.ShadowAmount == "x10" then amtIdx = 3 end
                
                ClickDialogButtonByIndex(amtIdx)
            end
        end
    end
end)
