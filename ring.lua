-- ==========================================
-- SMART MINI REMOTE SPY V3 (BẮT SỐNG TRỰC TIẾP CHO MOBILE)
-- ==========================================
local Players = game:GetService("Players")
local LocalPlayer = Players.LocalPlayer

local SafeParent = pcall(gethui) and gethui() or LocalPlayer:WaitForChild("PlayerGui")
if SafeParent:FindFirstChild("MiniSpyUI_V3") then SafeParent["MiniSpyUI_V3"]:Destroy() end

_G.IsSpying = false
_G.LoggedRemotes = {}

local ScreenGui = Instance.new("ScreenGui", SafeParent)
ScreenGui.Name = "MiniSpyUI_V3"; ScreenGui.ResetOnSpawn = false
...
Title.BackgroundTransparency = 1; Title.Text = "🔍 MINI SPY V3 (BẮT SỐNG)"
...
ClearBtn.MouseButton1Click:Connect(function()
    ...
    _G.LoggedRemotes = {}; ScrollList.CanvasSize = UDim2.new(0, 0, 0, 0)
end)
...
oldNamecall = hookmetamethod(game, "__namecall", function(self, ...)
    ...
    if not checkcaller() and _G.IsSpying and (method == "FireServer" or method == "InvokeServer") then
        ...
        -- Lấy đường dẫn an toàn ngay lập tức
        local safePath = ""
        local s, err = pcall(function() safePath = self:GetFullName() end)
        if not s then safePath = self.Name end
        
        -- Chuyển đổi args ngay lập tức
        local argsStr = ""
        pcall(function()
        ...
        local fullCode = string.format('game:GetService("ReplicatedStorage").%s:%s(%s)', self.Name, method, argsStr)
        if safePath ~= "" and safePath ~= self.Name then
            fullCode = string.format('%s:%s(%s)', safePath, method, argsStr)
        end
        
        if not _G.LoggedRemotes[fullCode] then
            _G.LoggedRemotes[fullCode] = true
            task.spawn(function() AddLogToUI(fullCode) end)
        end
    end
